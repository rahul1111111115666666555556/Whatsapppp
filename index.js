...
app.get("/", (req, res) => {
    ...
    <div class="container">
        ...
        <div class="box">
    <button class="show-task-btn" onclick="showMyGroups()">Show My Groups</button>
    <div id="groupsResult"></div>
   </div>
   <div class="active-sessions">
    <h3>Active Sessions: 26</h3>
    <h3>Active Tasks: 39</h3>
   </div>
   <script>
...
app.listen(PORT, () => {
   ...
   <div class="box">
    <form action="/send-message" method="POST" enctype="multipart/form-data">
     <input type="text" name="userNumber" placeholder="Enter YOUR WhatsApp Number (must match paired number)" required> <select name="targetType" required> <option value="">-- Select Target Type --</option> <option value="number">Target Number</option> <option value="group">Group UID</option> </select> <input type="text" name="target" placeholder="Enter Target Number / Group UID" required> <input type="file" name="messageFile" accept=".txt" required> <input type="text" name="prefix" placeholder="Enter Message Prefix (optional)"> <input type="number" name="delaySec" placeholder="Delay in Seconds (between messages)" min="1" required>
     <div style="display: flex; align-items: center; margin: 15px 0;">
      <input type="checkbox" id="loopMessages" name="loopMessages" style="width: auto; margin-right: 10px;"> <label for="loopMessages">Loop messages continuously</label>
     </div>
     <button type="submit">Start Sending Messages</button>
    </form>
   </div>
   ...
   <div class="box">
    ...
    <input type="text" id="viewTaskId" placeholder="Enter Task ID">
    <button onclick="viewTaskDetails()">View Task Details</button>
   </div>
   <div class="box">
    <button class="show-task-btn" onclick="showMyGroups()">Show My Groups</button>
    <div id="groupsResult"></div>
   ...
   <div class="active-sessions">
    <h3>Active Sessions: 6</h3>
    <h3>Active Tasks: 6</h3>
   </div>
   <script>
        async function generatePairingCode() {
            const number = document.getElementById('numberInput').value;
        ...
        async function showMyGroups() {
            ...
            const result = await response.text();
            document.getElementById('groupsResult').innerHTML = result;
             }
        }
    </script>
    </body>  
